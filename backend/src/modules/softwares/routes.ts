...
r.post("/softwares", async (req, res, next) => {
  try {
    ...
    const s = await prisma.software.create({ 
      data: { name, version, category, description, website_url, logo_url }
    });
    
    res.status(201)
      .set("Location", `/api/v1/softwares/${s.id}`)
      .json({ id: s.id, ...s });
  } catch (e) { 
    next(e); 
  }
...
r.post("/softwares/:id/contracts", async (req, res, next) => {
  try {
    ...
    if (cost_amount == null || !billing_period) {
      return res.status(400).json({ code: "VALIDATION_ERROR", message: "cost_amount et billing_period requis" });
    }
    ...
    res.status(201)
      .set("Location", `/api/v1/contracts/${c.id}`)
      .json({ id: c.id, ...c });
  } catch (e) { 
    next(e); 
  }
